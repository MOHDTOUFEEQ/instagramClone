...
function Onlymessages({ id, name ,profile ,profile1}) {
  ...
  useEffect(() => {
    if (id) {
      ...
      // Set up interval to fetch messages every 10 seconds (adjust as needed)
  ...
  return (
    ...
    { !id? 
   <div className="h-screen overflow-y-auto p-4 flex justify-center align-center" style={{ width: '70vw', display:"flex", alignItems:"center" }}>
      <div class="bg-white p-4 rounded-lg">
      <h2 class="text-xl font-semibold mb-2" style={{textAlign: "center"}}>Send private photos and messages</h2>
      <p class="text-gray-600 mb-4" style={{textAlign:"center"}}>Start a conversation with one or more friends using Your Messages.</p>
      <button class="bg-blue-500 text-white px-4 py-2 rounded-lg" style={{margin:"0 25%"}}>Start a conversation now</button>
    </div>
  </div>
    : null
